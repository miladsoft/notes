...
export class PostComponent implements OnInit {
    ...
    async sendFollowEvent(unfollow = false) {
      if (this.post) {
          ...
          if (unfollow) {
              tags = tags.filter(tag => tag[1] !== this.post?.pubkey);
          } else {
  ...
  async sendLikeEvent() {
    ...
    copyLink() {
        if (this.post) {
            let link = `https://notes.seniorblockchain.io/#/posts/${this.post.nostrEventId}`;
    ...
    async repost() {
      if (this.post) {
          const privateKey = this.signerService.getPrivateKey();
          let tags = [
              ["e", this.post.noteId, ""],  // tag for the event being reposted
              ["p", this.post.pubkey]       // tag for the pubkey of the original author
          ];
